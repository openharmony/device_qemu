...
#include "los_task.h"
#include "los_sched.h"
VOID LOS_TaskLockSave(UINT32 *intSave)
{
    *intSave = LOS_IntLock();
    g_losTaskLock++;
}

VOID LOS_TaskUnlockRestore(UINT32 intSave)
{
    if (g_losTaskLock > 0) {
        g_losTaskLock--;
        if (g_losTaskLock == 0) {
...
#define LOS_SpinLock(lock) LOS_TaskLock()
#define LOS_SpinUnlock(lock) LOS_TaskUnlock()
#define LOS_SpinLockSave(lock, intSave)  LOS_TaskLockSave(intSave)
#define LOS_SpinUnlockRestore(lock, intSave)  LOS_TaskUnlockRestore(intSave)
...
static void ReleaseRxEntry(struct pbuf *p)
{
    ...
    if (nic->dev.vq[0].used->flag != VIRTQ_USED_F_NO_NOTIFY) {
        FENCE_WRITE_UINT32(0, nic->dev.base + VIRTMMIO_REG_QUEUENOTIFY);
    }
}
...
static err_t ConfigRxBuffer(struct VirtNetif *nic, VADDR_T buf)
{
    ...
    for (i = 0; i < q->qsz; i++) {
        q->desc[i].pAddr = u32_to_u64(paddr);
...
static err_t LowLevelOutput(struct netif *netif, struct pbuf *p)
{
    ...
    trans->desc[head].pAddr = u32_to_u64(VMM_TO_DMA_ADDR((PADDR_T)&nic->vnHdr));
    ...
    while (q != NULL) {
        ...
        trans->desc[tmp].pAddr = u32_to_u64(VMM_TO_DMA_ADDR((PADDR_T)q->payload));
    ...
    DSB;
    trans->avail->index++;
    FENCE_WRITE_UINT32(1, nic->dev.base + VIRTMMIO_REG_QUEUENOTIFY);
...
static void VirtnetIRQhandle(HwiIrqParam *param)
{
    ...
    FENCE_WRITE_UINT32(VIRTMMIO_IRQ_NOTIFY_USED, nic->dev.base + VIRTMMIO_REG_INTERRUPTACK);
}
