...
static bool RequestResponse(unsigned queue, const void *req, size_t reqSize, volatile void *resp, size_t respSize)
{
    ...
    q->desc[idx].pAddr = u32_to_u64(VMM_TO_DMA_ADDR((VADDR_T)req));
    q->desc[idx].len = reqSize;
    q->desc[idx].flag = VIRTQ_DESC_F_NEXT;
    q->desc[idx].next = idx + 1;
    idx++;
    q->desc[idx].pAddr = u32_to_u64(VMM_TO_DMA_ADDR((VADDR_T)resp));
                                ...
                                size_t dataSize, volatile void *resp, size_t respSize)
{
    ...
    q->desc[idx].next = idx + 1;
    idx++;
    q->desc[idx].pAddr = u32_to_u64(VMM_TO_DMA_ADDR((VADDR_T)data));
...
static void RequestNoResponse(unsigned queue, const void *req, size_t reqSize, bool notify)
{
    ...
    q->desc[head].pAddr = u32_to_u64(VMM_TO_DMA_ADDR((VADDR_T)req));
...
{
   return g_virtGpu->screen.width * g_virtGpu->screen.height * PIXEL_BYTES;
}
...
static void PopulateVirtQ(void)
{
    ...
    for (n = 0; n < VIRTQ_NUM; n++) {
        ...
        for (i = 0; i < qsz; i += NORMAL_CMD_ENTRIES) {
            ...
            q->desc[i].next = i + 1;
            q->desc[i + 1].pAddr = u32_to_u64(VMM_TO_DMA_ADDR((VADDR_T)&g_virtGpu->resp));
...
static void VirtgpuDeInit(struct Virtgpu *gpu)
{
    ...
    if (gpu->fb) {
        LOS_MemFree(OS_SYS_MEM_ADDR, gpu->fb);
    }
    LOS_MemFree(OS_SYS_MEM_ADDR, gpu);
    g_virtGpu = NULL;
}
...
static struct Virtgpu *VirtgpuInitDev(void)
{
    ...
    if (!VirtmmioDiscover(VIRTMMIO_DEVICE_ID_GPU, &gpu->dev)) {
        HDF_LOGE("[%s]VirtmmioDiscover failed\n", __func__);
        goto ERR_OUT;
    }
    ...
    if (!VirtmmioNegotiate(&gpu->dev, Feature0, Feature1, gpu)) {
        HDF_LOGE("[%s]VirtmmioNegotiate failed\n", __func__);
        goto ERR_OUT1;
    }
    ...
    if (VirtmmioConfigQueue(&gpu->dev, base, qsz, VIRTQ_NUM) == 0) {
        HDF_LOGE("[%s]VirtmmioConfigQueue failed\n", __func__);
        goto ERR_OUT1;
    }
    ...
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("[%s]create timer failed: %d\n", __func__, ret);
        goto ERR_OUT1;
    }
...
static bool VirtgpuInitResourceHelper(uint32_t resourceId)
{
    ...
    if (resourceId == RESOURCEID_FB) {
        va = u32_to_u64((uint32_t)g_virtGpu->fb);
